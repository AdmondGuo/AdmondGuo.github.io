...
> 
>dad5f5f (HEAD -> master, origin/master, origin/HEAD) HEAD@{1}: rebase (continue) (pick): KN-190 fix finish time not set when discard jobs
> 
>1a2a888 (kyligence/master) HEAD@{2}: pull -r kyligence master (start): checkout 1a2a888f97ec81f5503954d74de021899b935c1f
> 
>742400e HEAD@{3}: reset: moving to HEAD
> 
>742400e HEAD@{4}: commit: KN-190 fix finish time not set when discard jobs
> 
>7581008 HEAD@{5}: merge origin/master: Fast-forward
> 
>91f4284 HEAD@{6}: reset: moving to 91f428415df5a11bb8bd0d6c907a89f7b5070792
> 
>7581008 HEAD@{7}: commit: KN-151 fix incorrect name when import notebook
> 
>53ddb28 HEAD@{8}: commit: KN-151 fix incorrect name when import notebook
> 
...
Date:   Mon Jul 5 17:41:42 2021 +0800
    ...
    KN-190 fix finish time not set when discard jobs

commit 7581008663c29ee2ec399c73a3ed4294b25058df
Author: admondguo <admondguo@163.com>
Date:   Wed Jun 30 18:07:28 2021 +0800

    KN-151 fix incorrect name when import notebook
...
Date:   Tue Jun 29 10:08:19 2021 +0800
    ...
    KN-179 resave when move file and folder
...
Date:   Mon Jun 28 14:12:25 2021 +0800
    ...
    KN-177  fix bind key when switch mode
:
    KN-179 resave when move file and folder
...
设置远端仓库（需要合入的参数）
```
TianqingGuodeMacBook-Pro:Notebook tianqing.guo$ git remote add git@github.com:Kyligence/Notebook.git kyligence
fatal: 'git@github.com:Kyligence/Notebook.git' is not a valid remote name
TianqingGuodeMacBook-Pro:Notebook tianqing.guo$ git remote add kyligence git@github.com:Kyligence/Notebook.git 
TianqingGuodeMacBook-Pro:Notebook tianqing.guo$ git remote
kyligence
origin
```
fetch 并 pull kyligence 仓库
```
TianqingGuodeMacBook-Pro:Notebook tianqing.guo$ git fetch kyligence
...
From github.com:Kyligence/Notebook
 * [new branch]      master     -> kyligence/master
 * [new branch]      test       -> kyligence/test
TianqingGuodeMacBook-Pro:Notebook tianqing.guo$ git pull -r kyligence master
...
Saved working directory and index state WIP on master: 742400e KN-190 fix finish time not set when discard jobs
```
## 2.合并操作
pull一下kyligence仓库，r代笔rebase
```
TianqingGuodeMacBook-Pro:Notebook tianqing.guo$ git pull -r kyligence master
From github.com:Kyligence/Notebook
 * branch            master     -> FETCH_HEAD
Auto-merging console/src/main/java/io/kyligence/notebook/console/controller/NotebookController.java
CONFLICT (content): Merge conflict in console/src/main/java/io/kyligence/notebook/console/controller/NotebookController.java
error: could not apply 53ddb28... KN-151 fix incorrect name when import notebook
Resolve all conflicts manually, mark them as resolved with
"git add/rm <conflicted_files>", then run "git rebase --continue".
You can instead skip this commit: run "git rebase --skip".
To abort and get back to the state before "git rebase", run "git rebase --abort".
Could not apply 53ddb28... KN-151 fix incorrect name when import notebook
```
发现有冲突，打开IDE解决冲突
查看log
```
TianqingGuodeMacBook-Pro:Notebook tianqing.guo$ git log
commit 1a2a888f97ec81f5503954d74de021899b935c1f (HEAD, kyligence/master)
...
Date:   Mon Jul 5 18:32:45 2021 +0800
    ...
    KN-188 create folder in target folder
...
Last command done (1 command done):
   pick 53ddb28 KN-151 fix incorrect name when import notebook
Next commands to do (2 remaining commands):
   pick 7581008 KN-151 fix incorrect name when import notebook
   pick 742400e KN-190 fix finish time not set when discard jobs
...
dropping 7581008663c29ee2ec399c73a3ed4294b25058df KN-151 fix incorrect name when import notebook -- patch contents already upstream
...
commit 1a2a888f97ec81f5503954d74de021899b935c1f (kyligence/master)
Merge: 45f8f2f 184fb9e
Author: Qian Xia <lauraxiaqian@gmail.com>
Date:   Mon Jul 5 18:32:45 2021 +0800
...
```
可以看到之前比kyligence仓库多出的分支已经合并成了一个分支
